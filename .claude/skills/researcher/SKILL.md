...
| cafe / restaurant | Google Places API | まちかわと同一の資産。place_id必須。営業時間は収集しない(変動が速く誌面に載せない)。写真があれば1枚を `image` に採用する(下記「Places写真の扱い」) |
...
**Places写真の扱い**: cafe/restaurant/architectureなどをPlacesで場所として引くとき、写真を1枚だけ `image` に採用してよい。取得手順は以下に限る(これ以外のPlaces画像取得は禁止):

1. Places検索の field mask に `places.photos` を含める(`authorAttributions` を含む写真オブジェクトが返る)
2. 先頭の写真 `photos[0]` について `https://places.googleapis.com/v1/{photo.name}/media?maxWidthPx=1200&key={KEY}&skipHttpRedirect=true` を叩き、返る**キー無しの `photoUri`** を `image.url` に入れる(`source: "google-places"`)。このmediaリクエストURLはキーを含むが**フェッチのみ**で、ファイル(`desk/` 配下含む)に書き残すのはキー無しの `photoUri` だけである(鉄則1の「URL組み立て」には当たらない、Placesで唯一許可された画像取得。iTunesの解像度置換と同格の例外)
3. `photos[0].authorAttributions[]` の `displayName` と `uri` を `image.attributions[]` に転記する。**撮影者クレジットの表示はGoogleのポリシー上必須**であり、`attributions` を欠いたPlaces写真は誌面に出せない(art-directorが画像下にクレジットを出し、koetsuが欠落を権利リスクとして弾く)
4. `photoUri` を `verify_links.py` で叩き、生存(200)を確認してから採用する。落ちていれば `image` を付けない
5. 写真が無い場所は `image` を付けない(openBDの空coverと同じ沈黙省略)
