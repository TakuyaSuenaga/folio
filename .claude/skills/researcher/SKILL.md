...
| genre | 主API | 備考 |
|---|---|---|
| book / poetry | NDLサーチ + openBD(いずれもキー不要) | `https://ndlsearch.ndl.go.jp/api/opensearch?any={query}&cnt=20` で検索してISBNを得て、`https://api.openbd.jp/v1/get?isbn={isbn}` で照合する。openBDにヒットしない書籍は候補にしない。source_api は "ndl+openbd"、source_id はISBN。書影は使わない |
