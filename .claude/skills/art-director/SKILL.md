...
- 外部リソースは Google Fonts と、JSONの `image.url`(openBD書影・iTunesジャケット・Places写真など、パイプラインが許可したもの)のみ。それ以外の画像・CDN・スクリプトの参照は禁止。
...
3. **作品セクション × N**: `items` の順序通り。各セクションは:
   ...
   - 本文: `essay` 全文
   - 画像: `image` があれば `alt` 付きで表示する(店・建築のPlaces写真も同様に載せる)。`alt` はJSONにその文字列があればそのまま使い、無ければ `title` から機械的に組む(例: 書影は「『title』書影」、ジャケットは「title ジャケット」、写真は「title の写真」)。`image` が無ければ入れない(タイポグラフィで見せるのが基本姿勢。画像の無い店・建築はテキストだけで組む)
   - 画像クレジット: `image.attributions` があれば画像の直下に小さく撮影者クレジットを出す(例: `写真: {name}`、`uri` があればリンク)。Places写真の帰属表示はGoogleのポリシー上必須なので、`attributions` があるとき描画は省略できない
   - **DATA欄**(下記)
   - **ノンブル・柱**(下記)
...
Places写真(`image.source: "google-places"`)を載せた号は、奥付に「Powered by Google」を一度添える(Google Placesのデータをマップ無しで掲載する場合のポリシー要件。写真の点数に関わらず誌面に1回でよい)。
...
- [ ] Places写真(`image.source: "google-places"`)は画像直下に撮影者クレジットがあり、奥付に「Powered by Google」がある
