...
- 外部リソースは Google Fonts と、JSONの `image.url`(openBD書影・iTunesジャケットなど、パイプラインが許可したもの)のみ。それ以外の画像・CDN・スクリプトの参照は禁止。
...
3. **作品セクション × N**: `items` の順序通り。各セクションは:
   ...
   - 本文: `essay` 全文
   - 画像: `image` があれば `alt` 付きで。`alt` はJSONにその文字列があればそのまま使い、無ければ `title` から機械的に組む(例: 書影は「『title』書影」、ジャケットは「title ジャケット」)。`image` が無ければ入れない(タイポグラフィで見せるのが基本姿勢。店・建築はテキストだけで組む)
   - **DATA欄**(下記)
   - **ノンブル・柱**(下記)
